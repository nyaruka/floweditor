...
import { Type, getTypeConfig, ADD_GROUP_ACTION_TYPE, REMOVE_GROUP_ACTION_TYPE } from '../../../config';
import { transformGroups, getLocalGroups } from './AddGroupForm';
...
const addGroupConfig: Type = getTypeConfig(ADD_GROUP_ACTION_TYPE);
const removeGroupConfig: Type = getTypeConfig(REMOVE_GROUP_ACTION_TYPE);
...
const removeAllGroupsAction: ChangeGroup = {
    ...removeGroupsAction,
    groups: []
};
...
describe('RemoveGroupForm >', () => {
    describe('render >', () => {
        ...
        it("should render only the 'Remove from Group' checkbox element when it's checked", () => {
            ...
            expect(
                getSpecWrapper(wrapper, 'field-container').children()
            ).toHaveLength(1);
        ...
        it('should keep groups in state, render GroupElement if CheckBoxElement checked and then unchecked', () => {
            ...
            const searchResults: SearchResult[] = transformGroups(
                removeGroupsAction.groups
            );

            expect(wrapper.find('GroupElement').prop('groups')).toEqual(
                searchResults
            );
    ...
    describe('instance methods >', () => {
        ...
        describe('getFields >', () => {
            it("should call ComponentMap prop's 'getGroups' method", () => {
                // Reference for the below: https://github.com/facebook/jest/issues/936#issuecomment-214556122
                const getLocalGroupsMock = jest.fn();

                jest.unmock('./AddGroupForm');

                const AddGroupFormModule = require('./AddGroupForm');

                AddGroupFormModule.getLocalGroups = getLocalGroupsMock;
                ...
                expect(getLocalGroupsMock).toHaveBeenCalledTimes(1);
            });
