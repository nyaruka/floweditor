...
const getGroupActionMarkup = ({ type, groups }: ChangeGroup): JSX.Element[] => {
    ...
    if (type === 'remove_from_group' && !groups.length) {
        ...
        const groupEls: JSX.Element[] = groups.reduce(
            (groupList, { name }, idx) => {
                if (idx <= 2) {
...
const ChangeGroupComp: React.SFC<ChangeGroup> = (props): JSX.Element => {
    const content: JSX.Element[] = getGroupActionMarkup(props);
