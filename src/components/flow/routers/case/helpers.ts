...
export const validateCase = (keys: {
  ...
  if (keys.operatorConfig.operands > 0) {
    ...
    if (keys.operatorConfig.type === Operators.has_number_between) {
      updates.min = validate(
        ...
        validators.concat([
          Numeric,
          LessThan(parseFloat(keys.max), "the maximum")
        ])
      );
      ...
      updates.max = validate(
        ...
        validators.concat([
          Numeric,
          MoreThan(parseFloat(keys.min), "the minimum")
        ])
      );
  ...
  updates.categoryName = validate(
    ...
    updates.argument.value || (updates.min.value && updates.max.value)
      ? [Required]
      : []
  );
...
export const getCategoryName = (state: Partial<CaseElementState>): string => {
  ...
  if (isRelativeDate(state.operatorConfig.type)) {
    ...
    if (!isNaN(count)) {
      const today =
        state.operatorConfig.type === Operators.has_date_eq ? "Today" : "today";
      const op = count < 0 ? " - " : " + ";
      const days = Math.abs(count) === 1 ? " day" : " days";
      return (
        prefix(state.operatorConfig.type) + today + op + Math.abs(count) + days
      );
    }
  }
