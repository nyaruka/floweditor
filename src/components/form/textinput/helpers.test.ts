...
const optionQueryMap = COMPLETION_VARIABLES.reduce(
  (argMap, option: CompletionOption) => {
...
describe("helpers", () => {
  ...
  describe("filterOptions", () => {
    it("should return top-level options if not passed a query", () =>
      expect(filterOptions(COMPLETION_VARIABLES, "", false)).toEqual(
        TOP_LEVEL_OPTIONS
      ));
    ...
    Object.keys(optionQueryMap).forEach(query =>
      it(`should filter options for "${query}"`, () =>
        expect(
          filterOptions(COMPLETION_VARIABLES, query, true)
        ).toMatchSnapshot())
    );
  });
  ...
  describe("getOptionsList", () => {
    it("should include only our predefined options if autocomplete arg is falsy", () => {
      expect(getCompletionOptions(false, {}).length).toBe(
        COMPLETION_VARIABLES.length
      );
    });
