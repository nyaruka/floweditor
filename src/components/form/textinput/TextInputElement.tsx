...
> {
  ...
  public componentWillReceiveProps(nextProps: TextInputProps): void {
    if (
      nextProps.entry.value !== this.props.entry.value ||
      this.nextCaret > -1
    ) {
  ...
  private handleChange({
    ...
    const updates: Partial<TextInputState> = this.executeQuery(
      value,
      selectionStart
    );
    ...
    if (this.props.autocomplete) {
      if (this.props.count === Count.SMS) {
        const { parts, characterCount, unicodeChars } = getMsgStats(
          value,
          true
        );
        updates.parts = parts;
        ...
        <div data-spec="option-example" className={styles.option_examples}>
          ...
          {option.examples
            ...
            .map((example: FunctionExample, idx: number) => (
              <div key={option.name + "_example_" + idx}>
                {" "}
                {example.template}
              </div>
            ))}
