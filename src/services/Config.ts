...
export class Config {
    ...
    private typeConfigMap: TypeConfigMap = {};
    ...
    private constructor(config: FlowEditorConfig) {
        ...
        /** Create a TypeConfig mapping for quick lookups */
        ...
        this.actionTypes = this.typeConfigs
            .filter(({ form: { prototype }}) => !(prototype instanceof SwitchRouterForm));
    }
    ...
    public getOperatorConfig(type: string): Operator {
        const foundOp = this.operators.find(({ type: opType }) => opType == type);
        if (!foundOp) {
